  ...
  Widget _buildPersonalSummary(String? uid) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, groupSnapshot) {
        ...
        userGroupIds.add('personal'); // Allow personal expenses
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            List<Map<String, dynamic>> myRelevantExpenses = [];
            ...
            for (var doc in docs) {
              ...
              final Map<String, dynamic>? paidContributions = data['paidContributions'] as Map<String, dynamic>?;

              // 1. Calculate how much the logged-in user paid for this expense
              double myPaid = 0.0;
              ...
              // 2. Calculate the logged-in user's fair share of this expense
              double myShare = (shares != null && shares.containsKey(uid))
                  ? (shares[uid] as num?)?.toDouble() ?? 0.0
                  : 0.0;

              // Filter: Only include if the user is a member of the group OR directly involved
              final bool isMemberOfGroup = userGroupIds.contains(groupId);
              final bool isDirectlyInvolved = myPaid > 0 || myShare > 0 || paidBy == uid;

              if (!isMemberOfGroup && !isDirectlyInvolved) {
                continue; // Skip expenses belonging to other groups
              }

              // Track expenditures paid by user
              if (myPaid > 0) {
                totalSpentByMe += myPaid;
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('My Payments in Joined Groups', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  const SizedBox(height: 8),

                  // 3. Paid Expenses List
                  Expanded(
                    child: myRelevantExpenses.isEmpty
                        ? const Center(child: Text('No group expenses paid by you yet.'))
                        : ListView.builder(
                            itemCount: myRelevantExpenses.length,
                            itemBuilder: (context, index) {
                              final item = myRelevantExpenses[index];
                ...
                if (expenseSnapshot.hasData) {
                  for (var doc in expenseSnapshot.data!.docs) {
                    ...
                    if (paidContributions != null && paidContributions.isNotEmpty) {
                      paidContributions.forEach((mId, pVal) {
                        final val = (pVal as num?)?.toDouble() ?? 0.0;
                        memberPaid[mId] = (memberPaid[mId] ?? 0) + val;
                      });
